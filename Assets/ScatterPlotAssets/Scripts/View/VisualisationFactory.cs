...
public class VisualisationFactory : MonoBehaviour
{
    ...
    /// <returns></returns>
    public Tuple<GameObject, SPView> CreateSingle2DView(DataObject dobjs, int DimensionX, int DimensionY, int DimensionZ, int LinkIndex, MeshTopology topology, Material m, bool parallel = false)
    {
        ...
        SPView v = new SPView(mtp, viewName);
        ...
        if (!parallel)
        {
            ...
            if (DimensionX >= 0)
            {
                ...
                v.setDataDimension(xpts, SPView.VIEW_DIMENSION.X);
            }
            if (DimensionY >= 0)
            {
                v.setDataDimension(dobjs.getDimension(DimensionY), SPView.VIEW_DIMENSION.Y);
            }
            if (DimensionZ >= 0)
            {
                v.setDataDimension(dobjs.getDimension(DimensionZ), SPView.VIEW_DIMENSION.Z);
            }
        }
        else
        {
            if (DimensionX >= 0 && DimensionY >= 0)
            {
                ...
                v.setDataDimension(data.ToArray(), SPView.VIEW_DIMENSION.Y);
                v.setDataDimension(range.ToArray(), SPView.VIEW_DIMENSION.X);
            }
        }
        if (LinkIndex < 0)
            v.updateView(null);
        ...
        return new Tuple<GameObject, SPView>(view, v);
    }

    public Tuple<GameObject, SPView> CreateLinkedPCPto2DScatterplotsViews(DataObject dobjs, int DimensionX1, int DimensionY1, int DimensionX2, Material m)
    {
        string viewName = "";
        ...
        SPView v = new SPView(MeshTopology.Lines, viewName);
        GameObject view = new GameObject(viewName);
        v.initialiseDataView(dobjs.DataPoints * 2, view);
        ...
        v.setDataDimension(XData.ToArray(), SPView.VIEW_DIMENSION.X);
        v.setDataDimension(YData.ToArray(), SPView.VIEW_DIMENSION.Y);
        v.setDataDimension(ZData.ToArray(), SPView.VIEW_DIMENSION.Z);
        v.updateView(null);
    ...
    public Tuple<GameObject, SPView> CreateLinked2DScatterplotsViews(DataObject dobjs, int DimensionX1, int DimensionY1, int DimensionX2, int DimensionY2, Material m)
    {
    ...
    public Tuple<GameObject, SPView> CreateLinked3DScatterplotsViews(DataObject dobjs, int DimensionX1, int DimensionY1, int DimensionZ1, int DimensionX2, int DimensionY2, int DimensionZ2, Material m)
    {
        float[] x1 = dobjs.getDimension(DimensionX1);
        float[] y1 = dobjs.getDimension(DimensionY1);
