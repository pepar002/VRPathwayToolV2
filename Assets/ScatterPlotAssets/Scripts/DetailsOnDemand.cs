...
public class DetailsOnDemand : MonoBehaviour
{
    ...
    public void OnDetailOnDemand2D()
    {
        ...
        Vector2 rangeX = ScatterPlotSceneManager.Instance.dataObject.DimensionsRange[ScatterPlotSceneManager.Instance.dataObject.dimensionToIndex(xDimension)];
        Vector2 rangeY = ScatterPlotSceneManager.Instance.dataObject.DimensionsRange[ScatterPlotSceneManager.Instance.dataObject.dimensionToIndex(yDimension)];
        string values = "";
        ...
        if (pointerPosition != null && labelDetails != null && textMesh != null)
        {
            if (!isParallelView)
            {
                ...
                for (int i = 0; i < ScatterPlotSceneManager.Instance.dataObject.getDimension(0).Length; i++)
                {
                    distances.Add(Vector2.Distance(pointerPosition2D, new Vector2(ScatterPlotSceneManager.Instance.dataObject.getDimension(xDimension)[i], ScatterPlotSceneManager.Instance.dataObject.getDimension(yDimension)[i])));
                }
                int index = distances.FindIndex(d => d < distances.Min() + precisionSearch && d > distances.Min() - precisionSearch);

                var dataObj = ScatterPlotSceneManager.Instance.dataObject;
                ...
                leaderInformation.SetPosition(0, pointerPosition);
                    leaderInformation.SetPosition(1,
                      (transform.TransformPoint((ScatterPlotSceneManager.Instance.dataObject.getDimension(xDimension)[index] - 0.5f) * 0.2660912f,
                       (ScatterPlotSceneManager.Instance.dataObject.getDimension(yDimension)[index] - 0.5f) * 0.2660912f, 0f)));
                    leaderInformation.widthCurve = AnimationCurve.Linear(0, 0.0015f, 1, 0.0015f);
    ...
    public void OnDetailOnDemand3D()
    {
        ...
        Vector2 rangeX = ScatterPlotSceneManager.Instance.dataObject.DimensionsRange[ScatterPlotSceneManager.Instance.dataObject.dimensionToIndex(xDimension)];
        Vector2 rangeY = ScatterPlotSceneManager.Instance.dataObject.DimensionsRange[ScatterPlotSceneManager.Instance.dataObject.dimensionToIndex(yDimension)];
        Vector2 rangeZ = ScatterPlotSceneManager.Instance.dataObject.DimensionsRange[ScatterPlotSceneManager.Instance.dataObject.dimensionToIndex(zDimension)];
            ...
            if (!isParallelView)
            {
                ...
                for (int i = 0; i < ScatterPlotSceneManager.Instance.dataObject.getDimension(0).Length; i++)
                {
                    var m = Vector3.SqrMagnitude(pointerPosition3D -
                        new Vector3(
                        ScatterPlotSceneManager.Instance.dataObject.getDimension(xDimension)[i],
                        ScatterPlotSceneManager.Instance.dataObject.getDimension(yDimension)[i],
                        ScatterPlotSceneManager.Instance.dataObject.getDimension(zDimension)[i]));
    ...
    {
        if (ScatterPlotSceneManager.Instance.dataObject.TypeDimensionDictionary1[dimension] == "string")
        {
            Vector2 range = ScatterPlotSceneManager.Instance.dataObject.DimensionsRange[dimension];
            float lerpedValue = Mathf.Lerp(range.x, range.y, value);
            float closest = UtilMath.ClosestTo(ScatterPlotSceneManager.Instance.dataObject.TextualDimensions.Keys.ToList(), lerpedValue);
            return ScatterPlotSceneManager.Instance.dataObject.TextualDimensions[closest].ToString();
        }
        else
            return ScatterPlotSceneManager.Instance.dataObject.getOriginalValue(value, dimension);
    }
